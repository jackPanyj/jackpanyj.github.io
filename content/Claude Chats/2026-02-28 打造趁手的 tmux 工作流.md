...
完整代码见文末「快速上手」部分。这里说明几个关键设计点。

### 数据源

列表数据来自两处：

- **已有 session** — `tmux ls` 输出，过滤掉关联 session（`-f '#{!=:#{session_grouped},1}'`）
...
- **已有 session** — 默认颜色显示路径名
- **zoxide 目录** — 灰色显示，视觉上一眼区分
- 内部用 tab 作为分隔符（`$'\t'`），配合 `--with-nth=1` 只显示路径列，干净无多余字符
- 关联 session 从列表中隐藏，不会看到 `jack-4` 这种临时 session

### 功能

| 操作 | 效果 |
|------|------|
| 选择已有 session | 通过关联 session 连接（共享窗口，独立视图） |
...
### 关联 Session（Grouped Session）

默认情况下，多个终端 attach 到同一个 session 会**完全同步**——终端 A 切到窗口 2，终端 B 也会被强制切到窗口 2，而且 session 尺寸会被压缩到最小的那个终端。

`tm` 使用了 tmux 的关联 session（`new-session -t`）来解决这个问题。关联 session 的特点：

- **共享窗口** — 关联 session 和原 session 共享同一组窗口，一方新建/关闭窗口，另一方也能看到
- **独立视图** — 每个终端可以独立选择查看哪个窗口，互不影响

举个例子，`~/sonic/manic-trade-web` 的 session 里有 3 个窗口：

| | 窗口 1 (nvim) | 窗口 2 (dev server) | 窗口 3 (git) |
|---|---|---|---|
| 终端 A | **正在看** | | |
| 终端 B | | **正在看** | |
...
_tm_attach() {
  ...
  if [[ -n "$TMUX" ]]; then
    local linked
    linked=$(tmux new-session -d -t "$target" -P -F '#{session_name}' 2>/dev/null)
    if [[ -n "$linked" ]]; then
    ...
    tmux new-session -t "$target" \; set-option destroy-unattached on
  fi
}
```
...
- `new-session -t "$target"` — 创建一个与目标 session 关联的新 session，共享窗口但独立视图
- `destroy-unattached on` — 关联 session 在终端关闭或断开后自动销毁，不会堆积
- tmux 内部用 `switch-client` 切换，外部直接创建并 attach
...
- **关联 Session** — 多终端访问同一 session 时共享窗口但独立视图，自动清理
...
# ── _tm_attach: connect via grouped session (shared windows, independent view) ─────
_tm_attach() {
  local target="$1"
  if [[ -n "$TMUX" ]]; then
    local linked
...
# - Existing sessions + zoxide directories in one list
# - New directories shown in gray for visual distinction
# - Grouped sessions are hidden from the list
# - ctrl-d to kill a session
tm() {
  ...
  # Clean up stale grouped sessions (no clients attached)
  tmux ls -F '#{session_name} #{session_attached} #{session_grouped}' 2>/dev/null | while read -r s a g; do
    [[ "$g" == "1" && "$a" == "0" ]] && tmux kill-session -t "=$s" 2>/dev/null
  done
  ...
  local list
  list=$(
    tmux ls -f '#{!=:#{session_grouped},1}' -F "#{session_name}${sep}#{session_path}" 2>/dev/null
    existing=$(tmux ls -f '#{!=:#{session_grouped},1}' -F '#{session_path}' 2>/dev/null)
    zoxide query -l 2>/dev/null | while read -r dir; do
      echo "$existing" | command grep -qxF "$dir" || printf '\033[90m%s\033[0m\t%s\tnew\n' "${dir/#$HOME/~}" "$dir"
    done
  )
  ...
  sel=$(echo "$list" | fzf \
    ...
    --delimiter=$'\t' \
    --with-nth=1 \
    --bind="ctrl-d:execute-silent(tmux kill-session -t {1} 2>/dev/null)+reload(tmux ls -f '#{!=:#{session_grouped},1}' -F '#{session_name}'$'\t''#{session_path}' 2>/dev/null)" \
    --preview='
      tag=$(echo {} | cut -f3)
      if [ "$tag" != "new" ]; then
        name=$(echo {} | cut -f1)
